...
const run = () => {
    try {
        const errors = [];
        const { snowUsername = '', snowPassword = '', snowSourceInstance = '', appSysID = '', appScope = '', GITHUB_WORKSPACE = '', GITHUB_RUN_NUMBER = '', GITHUB_TOKEN = '', } = process.env;
        ...
        if (!GITHUB_TOKEN) {
            errors.push(App_types_1.Errors.GITHUB_TOKEN);
        }
        if (!GITHUB_WORKSPACE) {
